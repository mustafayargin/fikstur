  ...
  const ADMIN_SYNC_DIAGNOSTICS_STORAGE_KEY =
    "fikstur_admin_sync_diagnostics_v1";
  const USERS_CACHE_STORAGE_KEY = "fikstur_users_cache_v1";
  const MATCHES_CACHE_STORAGE_KEY = "fikstur_matches_cache_v1";
  const PREDICTIONS_CACHE_STORAGE_KEY = "fikstur_predictions_cache_v1";

  const USERS_CACHE_MAX_AGE_MS = 1000 * 60 * 60 * 12; // 12 saat
  const MATCHES_CACHE_MAX_AGE_MS = 1000 * 60 * 15; // 15 dakika
  const PREDICTIONS_CACHE_MAX_AGE_MS = 1000 * 60 * 10; // 10 dakika
  ...
  function resetAppLoadingState() {
    ...
    if (els.message)
      els.message.textContent = APP_LOADING_DEFAULT_STATE.message;
    if (els.percent)
  ...
  function setAppLoading(show, options = {}) {
    ...
    if (options.message && els.message)
    ...
    if (typeof options.percent === "number") {
      const safePercent = Math.max(
        0,
        Math.min(100, Math.round(options.percent)),
      );
  ...
  function getHeaderSyncButtons() {
    return Array.from(
      document.querySelectorAll('[data-role="global-sync-btn"]'),
    );
  }
        ...
        if (currentMatch && currentPlayer) {
          upsertLocalPredictionRecord({
            ...
            points: currentMatch.played
              ...
              : 0,
            remoteId:
              result.id || result.predictionId || pred?.remoteId || null,
    ...
    try {
      ...
      return {
        ...
        queuedCountSnapshot: Number(
          parsed?.queuedCountSnapshot ||
            getPendingPredictionQueue().length ||
            0,
        ),
      };
    } catch {
  ...
  function jsonpRequest(action, params = {}) {
    return new Promise((resolve, reject) => {
      if (!navigator.onLine) {
        reject(new Error("İnternet bağlantısı yok."));
        return;
      }
  ...
  function setCachedData(storageKey, data) {
    try {
      ...
      console.warn("Cache yazılamadı:", storageKey, error);
    }
  }

  function getCachedData(storageKey, maxAgeMs = Infinity) {
    try {
      ...
      console.warn("Cache okunamadı:", storageKey, error);
      return null;
    }
  }

  function getAnyCachedData(storageKey) {
    try {
      const raw = localStorage.getItem(storageKey);
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      ...
      console.warn("Eski cache okunamadı:", storageKey, error);
      return null;
    }
  }
  ...
  function ensureSeasonFromOnlineLabel(seasonLabel, fallbackLeagueName = "") {
    ...
    if (!state.settings.activeSeasonId)
      state.settings.activeSeasonId = season.id;
    return season;
  }
  ...
  async function syncOnlineMatchesFromSheet(options = {}) {
    ...
    try {
      ...
      if (!rows.length) {
        const cachedMatches = getCachedData(
          MATCHES_CACHE_STORAGE_KEY,
          MATCHES_CACHE_MAX_AGE_MS,
        );
        if (!Array.isArray(cachedMatches) || !cachedMatches.length)
          return false;

        console.warn("Canlı maç verisi gelmedi, cache kullanılacak.");
        rows = cachedMatches;
      }
      ...
      setCachedData(MATCHES_CACHE_STORAGE_KEY, rows);
      ...
      const cachedRows = getCachedData(
        MATCHES_CACHE_STORAGE_KEY,
        MATCHES_CACHE_MAX_AGE_MS,
      );

      if (Array.isArray(cachedRows) && cachedRows.length) {
        console.warn("Maçlar cache üzerinden geri yüklendi.");
        const touchedWeekIds = new Set();
        let lastSeasonId = requestedSeasonId || null;
        ...
        cachedRows.forEach((row) => {
          ...
          let week = state.weeks.find(
            (item) =>
              item.seasonId === season.id && Number(item.number) === weekNumber,
          );
          ...
          const existingMatch =
            state.matches.find(
              (item) =>
                item.id ===
                String(row.localMatchId || row.matchId || row.id || ""),
            ) ||
            state.matches.find(
              (item) =>
                String(item.sheetMatchId || "") ===
                String(row.sheetMatchId || row.id || row.matchId || ""),
            );
          ...
          const normalizedMatch = {
            id:
              existingMatch?.id ||
              String(row.localMatchId || row.matchId || row.id || uid("match")),
            seasonId: season.id,
            weekId: week.id,
            homeTeam: row.homeTeam || row.evSahibi || "",
            awayTeam: row.awayTeam || row.deplasman || "",
            date: normalizeStoredDate(row.date || row.tarih || ""),
            played: parseBooleanish(row.played ?? row.oynandiMi),
            homeScore: homeScore === "" ? null : homeScore,
            awayScore: awayScore === "" ? null : awayScore,
            apiId: row.apiId || "",
            sheetMatchId: String(
              row.sheetMatchId || row.id || row.matchId || "",
            ),
          };

          if (existingMatch) {
            Object.assign(existingMatch, normalizedMatch);
          } else {
            state.matches.push(normalizedMatch);
          }
        });

        if (lastSeasonId && !state.settings.activeSeasonId) {
          state.settings.activeSeasonId = lastSeasonId;
        }
  ...
  async function syncUsersFromSheet(options = {}) {
    ...
    try {
      ...
      state.players = users;
      setCachedData(USERS_CACHE_STORAGE_KEY, users);
      ...
      const cachedUsers = getCachedData(
        USERS_CACHE_STORAGE_KEY,
        USERS_CACHE_MAX_AGE_MS,
      );

      if (Array.isArray(cachedUsers) && cachedUsers.length) {
        console.warn("Kullanıcılar cache üzerinden yüklendi:", error);
        state.players = cachedUsers;
  ...
  async function sendMatchesToSheet(matches) {
    if (!useOnlineMode || !Array.isArray(matches) || !matches.length)
      return null;
  ...
  function findPlayerForSessionUser(user = getAuthUser()) {
    ...
    let matched = playerList.find((item) =>
  ...
  async function syncOnlinePredictions(options = {}) {
    ...
    try {
      ...
      let rows = normalizeOnlinePredictionRows(response);

      if (!rows.length) {
        const cachedPredictionRows = getCachedData(
          PREDICTIONS_CACHE_STORAGE_KEY,
          PREDICTIONS_CACHE_MAX_AGE_MS,
        );
        if (
          Array.isArray(cachedPredictionRows) &&
          cachedPredictionRows.length
        ) {
          console.warn("Canlı tahmin verisi gelmedi, cache kullanılacak.");
          rows = cachedPredictionRows;
        }
      } else {
        setCachedData(PREDICTIONS_CACHE_STORAGE_KEY, rows);
      }
      ...
      if (Array.isArray(cachedRows) && cachedRows.length) {
        console.warn("Tahminler cache üzerinden geri yüklendi.");
  ...
  async function hydrateOnlineStateForSession(options = {}) {
    ...
    try {
      ...
      const cachedUsers = getAnyCachedData(USERS_CACHE_STORAGE_KEY);
      const cachedMatches = getAnyCachedData(MATCHES_CACHE_STORAGE_KEY);
      const cachedPredictions = getAnyCachedData(PREDICTIONS_CACHE_STORAGE_KEY);
      ...
      if (Array.isArray(cachedPredictions) && cachedPredictions.length) {
        restoredFromCache = true;
      }
      ...
      if (updateLoadingUi) {
        setAppLoading(true, {
          title: restoredFromCache
            ? "Çevrimdışı veri açıldı"
            : "Yükleme tamamlanamadı",
          message: restoredFromCache
            ? "Google bağlantısı kurulamadı. Son kayıtlı veriler açıldı."
            : error?.message ||
              "Google Sheets verileri alınırken bir sorun oluştu.",
          stepLabel: restoredFromCache
            ? "Bağlantı gelince veriler yeniden eşitlenebilir."
            : "Tekrar giriş yapabilir veya sayfayı yenileyebilirsin.",
          percent: 100,
          showSuccess: false,
        });
      }
  ...
  async function loginUser() {
    ...
    try {
      ...
      await runSessionHydrationWithFastOverlay({
        loadingMessage:
          "Kayıtlı veriler açılıyor, güncel bilgiler yükleniyor...",
      });
    } catch (error) {
          ...
          <div class="mobile-user-predictions">${players
            .map((player) => {
              ...
              const showDeleteAction =
                hasPrediction || pred.remoteId || isSaving;
              ...
              <div class="mobile-user-prediction premium-user-card ${pointLabel(pred.points)} ${outcomeClass} ${statusClass} ${lockedClass} ${ownClass}">
                <div class="mobile-user-head premium-user-head">
                  ...
                  ${match.played ? `<span class="mini-points premium-points prediction-points-badge ${getPredictionPointsBadgeClass(pred.points)}">${pred.points || 0} puan</span>` : ``}
                </div>
  ...
  function getPredictionPointsBadgeClass(points) {
    if (Number(points || 0) >= 3) return "badge-exact";
    if (Number(points || 0) >= 1) return "badge-close";
    return "badge-miss";
  }
  ...
  function renderWeekOptions(select, seasonId, includePlaceholder = false) {
    ...
    weeks.forEach((week) => {
      const selected =
        week.id === state.settings.activeWeekId ? "selected" : "";
      select.insertAdjacentHTML(
        "beforeend",
  ...
  async function syncDashboardWeek() {
    ...
    try {
      ...
      if (status)
        status.textContent = `${weekStatus} • ${getSyncSummaryText()}`;
      recordAdminSyncActivity({
  ...
  function renderStats() {
    ...
    const cards = [
      ...
      [
        "Eksik Tahmin",
        activeWeekId ? countMissingPredictions(activeWeekId) : 0,
      ],
      ...
      .map((match) => {
        ...
        const playerCols = players
          .map((player) => {
            ...
            const pointsBadgeClass = getPredictionPointsBadgeClass(pred.points);
            const bottomLabel =
              shareView === "post"
                ? hasPoints
                  ? `<div class="mini-points share-mini-note prediction-points-badge ${pointsBadgeClass}">${pred.points || 0} puan</div>`
                  : `<div class="share-mini-note share-mini-note--empty"></div>`
                : locked && !match.played
                  ? `<div class="mini-points share-mini-note share-mini-note-lock">Kilitli</div>`
                  : `<div class="share-mini-note share-mini-note--empty"></div>`;

            return `
        <div class="prediction-cell share-prediction-cell ${outcomeClass} ${pointLabel(pred.points)}${ownClass}${emptyClass}${fadedClass}">
          <div class="share-score-value share-score-pill">${value}</div>
          ${bottomLabel}
        </div>`;
          })
          .join("");
    ...
    const rows = matches
      .map((match) => {
        ...
        const playerCols = players
          ...
          ${match.played ? `<div class="mini-points top-left prediction-points-badge ${getPredictionPointsBadgeClass(pred.points)}">${pred.points || 0} puan</div>` : ``}
  ...
  function updatePredictionDeleteButton(
    matchId,
    playerId,
    forceVisible = null,
  ) {
  ...
  function getPredictionSaveLabel(matchId, playerId) {
    ...
    if (uiState === "saved")
      return hasSavedValue ? "Güncellendi" : "Kaydedildi";
  ...
  function setPredictionUiState(matchId, playerId, uiState) {
    ...
    const status = document.getElementById(
      `pred_status_${matchId}_${playerId}`,
    );
    if (status) {
      if (uiState === "saving") {
        ...
        status.textContent =
          "Silme sıraya alındı • Bağlantı gelince uygulanacak";
        status.dataset.saveState = "queued";
  ...
  function renderAdvancedStats() {
    ...
    document.getElementById("advancedStatsGrid").innerHTML = [
      ...
      [
        "En Çok Tam Skor",
        info.bestExact
          ? `${info.bestExact.name} (${info.bestExact.exact})`
          : "-",
      ],
      [
        "En Çok Doğru Sonuç",
  ...
  function addPlayer(buttonOrEvent) {
    ...
    if (
      state.players.some((p) => normalizeText(p.name) === normalizeText(name))
    )
  ...
  function parseApiEvent(item) {
    ...
    const roundMatch = String(item.intRound || item.strRound || "").match(
      /\d+/,
    );
